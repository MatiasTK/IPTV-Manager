...
const getActiveChannelsStmt = db.prepare(`
  SELECT id, url, name, health_status FROM channels WHERE is_active = 1
`);
 ...
 */
function checkUrl(urlStr, timeoutMs) {
  return new Promise((resolve) => {
    ...
    const options = {
      ...
      headers: {
        'User-Agent': 'IPTV-Manager-HealthCheck/1.0',
      },
      timeout: timeoutMs,
    };
...
async function checkChannels(channels, timeoutMs, concurrency) {
  ...
  async function worker() {
    while (idx < channels.length) {
      ...
      const result = await checkUrl(ch.url, timeoutMs);
...
async function checkSingleChannel(channelId) {
  const channel = db.prepare('SELECT id, url, name, health_status FROM channels WHERE id = ?').get(channelId);
  if (!channel) return null;

  const result = await checkUrl(channel.url, config.healthCheck.timeoutMs);
